...
def calculate_all_participant_ln_likelihood_and_update_hashmap(
    ...
    for p in range(n_participants):
        ...
        if p in non_diseased_participant_ids:
            this_participant_likelihood = new_utils.compute_likelihood(
            ...
            for k_j in diseased_stages:
                kj_likelihood = new_utils.compute_likelihood(
...
def metropolis_hastings_soft_kmeans(
    ...
    prior_theta_phi_estimates = new_utils.get_theta_phi_estimates(
        data_we_have)
    ...
    for _ in range(iterations):
        ...
        new_utils.shuffle_order(new_order, n_shuffle)
